...
function Process(process_id){
    ...
    this.id = process_id;
    this.running = false;
    this.resources = {requested : [], has : []};
	this.cords = {x : Process.cords[0][0], y : Process.cords[0][1]};
    this.dimension = {width: 30, height: 0};
	console.log('Process '+this.id+' was created');
    
    this.requestResource = function(resource_id){
        if(!this.isRequesting(resource_id)){
            this.resources.requested.push(resource_id);
        }
		console.log('Process '+this.id+' has requested resource '+resource_id);
    }
    
    this.addResource = function(resource_id){
        this.removeResource(resource_id);
        this.resources.has.push(resource_id);
        console.log('Process '+this.id+' has requested resource '+resource_id);
    }
    
    this.hasResource = function(resource_id){
        return (this.resource.has.indexOf(resource_id) != -1)? true : false;
    }
        
    this.isRequesting = function(resource_id){
        return (this.resource.requested.indexOf(resource_id) != -1)? true : false;
    }
    
    this.releaseResource = function(resource_id){
        this.removeResource(resource_id);
        this.requestResource(resource_id);
		console.log('Process '+this.id+' has requested resource '+resource_id);
    }
	...
	this.removeResource = function(resource_id){
        ...
        if(this.hasResource(resource_id)){
            index = this.resources.has.indexOf(resource_id);
		  this.resources.splice(index, 1);
        }
        if(this.isRequesting(resource_id)){
          index = this.resources.requested.indexOf(resource_id);
		  this.resources.splice(index, 1);
        }
	...
	this.draw = function(){
        this.elem = new Arc(this.cords.x, this.cords.y, this.dimension.width);
        this.elem.start;
...
function Resource(resource_id){
	...
	this.unit = 1;
	this.id = resource_id;
	this.processes = {requesting : [], assigned : []};	
	this.cords = {x : 400, y : Resource.top[0]};
    this.dimension = { width: 80, height : 80};
	console.log('Resource '+this.id+' was created');
	
    this.requestingProcess = function(process_id){
      if(!this.isRequesting(process_id)){
            this.processes.requesting.push(process_id);
        }
		console.log('Process '+this.id+' has requested resource '+process_id);   
    }
    
    this.assignProcess = function(process_id){
        if(this.isAssigned(process_id)){
            this.processes.assigned.push(process_id);
		  console.log('Process '+this.id+' has assigned to resource '+process_id);   
        }
	}
    
    this.isAssigned = function(process_id){        
        return (this.processes.assigned.indexOf(process_id) != -1)? true : false;
    }
    
    this.isRequesting = function(process_id){        
        return (this.processes.requesting.indexOf(process_id) != -1)? true : false;
    }
    
	this.removeProcess = function(process_id){
        var index;
        if(this.isAssigned(process_id)){
            index = this.processes.assigned.indexOf(resource_id);
		  this.processes.assigned.splice(index, 1);
        }
        if(this.isRequesting(process_id)){
          index = this.processes.requesting.indexOf(process_id);
		  this.processes.requesting.splice(index, 1);
        }
		console.log('Resource '+resource_id+' was removed from process '+this.id);
	}
